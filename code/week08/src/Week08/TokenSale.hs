...
import           Ledger.AddressMap            as AM
...
type LoveLaceInteger =  Integer
type TokenInteger    =  Integer
...
transition ts s r = case (stateValue s, stateData s, r) of
    ...
    (_, _, EndSale)                             -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
...
endTokenSale :: TokenSale -> Contract w s Text ()
endTokenSale ts = void $ mapErrorSM $ runStep (tsClient ts) EndSale
...
useEndpoints ts = forever
                $ handleError logError
                $ awaitPromise
                $ setPrice' `select` addTokens' `select` buyTokens' `select` withdraw' `select` endSale'
  where
    ...
    endSale'   = endpoint @"end token sale" $ const $ endTokenSale ts
