...
instance Eq GameDatum where
    {-# INLINABLE (==) #-}
    GameDatum byteString maybeChoice == GameDatum byteString' maybeChoice' = (byteString == byteString') && (maybeChoice == maybeChoice')
    Finished        == Finished          = True
    _               == _                 = False

PlutusTx.unstableMakeIsData ''GameDatum

data GameRedeemer = Play GameChoice | Reveal ByteString GameChoice | ClaimFirst | ClaimSecond | ClaimDraw
    deriving Show
...
gameDatum tranasctionOutput dathumHashFunction = do
    dathumHash      <- txOutDatum tranasctionOutput
    Datum datum     <- dathumHashFunction dathumHash
...
transition game state redeemer = case (stateValue state, stateData state, redeemer) of
    (value, GameDatum bytestring Nothing, Play choice)
        | lovelaces value == gStake game         -> Just ( 
                                                           Constraints.mustBeSignedBy (gSecond game) <> Constraints.mustValidateIn (to $ gPlayDeadline game)
                                                         , State (GameDatum bytestring $ Just choice) (lovelaceValueOf $ 2 * gStake game)
                                                         )

    (value, GameDatum _ (Just c), Reveal _ c')
        | lovelaces value == (2 * gStake game) &&
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce Rock) _ =
    sha2_256 (nonce `concatenate` bsRock') == bs
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce Paper) _ =
    sha2_256 (nonce `concatenate` bsPaper') == bs
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce Scissors) _ =
    sha2_256 (nonce `concatenate` bsScissors') == bs
...
firstGame fp = do
    ...
    let game   = Game
        ...
        bsResult = case c of
            ...
            Paper    -> bsPaper
            Scissors -> bsScissors
            _        -> "Error: Gamechoice not recognized"
        bs       = sha2_256 $ fpNonce fp `concatenate` bsResult
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            GameDatum _ Nothing -> do
                ...
                m' <- mapError' $ getOnChainState client
                case m' of
                    Nothing -> logInfo @String "first player won or tie"
                    Just (txOutt', s'') -> do
